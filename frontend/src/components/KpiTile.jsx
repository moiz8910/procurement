...
const KpiTile = ({ icon: Icon, label, value, sub, accent = 'emerald', onClick, badge, isDark }) => {
  const map = {
    ...
    slate:   'border-slate-200  bg-white [&_.ib]:bg-slate-100 [&_.ib]:text-slate-600',
    ...
    dark:    'border-slate-700  bg-slate-800 text-white [&_.ib]:bg-slate-700/50 [&_.ib]:text-emerald-400',
  };
  const theme = isDark ? map.dark : (map[accent] || map.emerald);
  
  return (
    <div
      className={`rounded-xl border p-4 flex flex-col gap-2 shadow-sm transition-all duration-200 ${theme} ${onClick ? 'cursor-pointer hover:-translate-y-0.5 hover:shadow-md' : ''}`}
      onClick={onClick}
    >
      ...
      <p className={`text-2xl font-black tracking-tight leading-none ${isDark ? 'text-white' : 'text-slate-800'}`}>{value ?? '—'}</p>
      {sub && <p className={`text-[10px] font-bold leading-tight ${isDark ? 'text-slate-400' : 'text-slate-400'}`}>{sub}</p>}
    </div>
  );
};
