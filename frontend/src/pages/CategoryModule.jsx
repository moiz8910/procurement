...
const FilterSelect = ({ label, value, onChange, children }) => (
  <div className="flex flex-col bg-slate-50 border border-slate-200 rounded-lg px-3 py-1.5 min-w-[140px]">
    <label className="text-[8px] font-black text-slate-400 uppercase tracking-widest">{label}</label>
    <select className="bg-transparent font-bold text-slate-800 outline-none text-xs" value={value} onChange={onChange}>
      {children}
    </select>
  </div>
);
...
const SectionHeader = ({ label }) => (
  <div className="flex items-center gap-3 pt-1">
    <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest whitespace-nowrap">{label}</p>
...
const IntelCard = ({ item, onClick }) => {
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-4 cursor-pointer hover:shadow-lg transition-all relative overflow-hidden group" onClick={onClick}>
      <div className="absolute inset-y-0 left-0 w-1 group-hover:w-1.5 transition-all" style={{ backgroundColor: col }} />
      <div className="ml-3 space-y-2">
        <div className="flex justify-between items-start gap-2">
          <h5 className="font-bold text-slate-800 text-sm leading-tight flex-1 group-hover:text-indigo-600 transition-colors uppercase tracking-tight">{item.title}</h5>
          <ImpactBadge impact={item.impact} />
        </div>
        <p className="text-xs text-slate-500 leading-relaxed line-clamp-2 font-medium">{item.desc}</p>
        <p className="text-[9px] uppercase font-bold tracking-widest text-slate-300 flex items-center gap-1"><Clock size={9}/> {item.time || 'JUST NOW'}</p>
      </div>
    </div>
  );
};
...
const CategoryModule = () => {
  ...
  return (
    <div className="max-w-[1600px] mx-auto px-4 py-4 space-y-4 text-slate-800">

      {/* ── 1. FILTER BAR ─────────────────────────────────────────────── */}
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-3 flex flex-wrap items-center gap-3">
        ...
        <div className="ml-auto flex items-center gap-4 bg-emerald-50/50 border border-emerald-100 rounded-xl px-4 py-2">
          <div className="text-right">
            ...
            <p className="text-xs font-black text-slate-800">{categoryOwner || 'Not Assigned'}</p>
          </div>
      ...
      {/* ── 3. TABS ───────────────────────────────────────────────────── */}
      <div className="flex items-center gap-1 bg-white p-1 rounded-xl border border-slate-100 w-fit shadow-sm">
        {[
          ...
          <button
            ...
            className={`flex items-center gap-2 px-6 py-2.5 rounded-lg font-black text-xs transition-md ${
              activeModTab === t.id ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-200' : 'text-slate-400 hover:bg-slate-50 hover:text-emerald-700'
            }`}
          >
      ...
      <div className="min-h-[600px]">
        ...
        {activeModTab === 'spend' && (
          <div className="bg-white rounded-3xl border border-slate-100 p-8 shadow-sm animate-in fade-in slide-in-from-bottom-2 duration-500">
             <div className="flex justify-between items-center mb-10">
                <div>
                  <h3 className="text-xl font-black text-slate-800">Spend Analytics Pulse</h3>
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Global Trend & Anomaly Observation</p>
                </div>
                <div className="flex bg-slate-50 p-1 rounded-xl border border-slate-200">
                  {['monthly','quarterly','yearly'].map(t => (
                    <button key={t} onClick={() => setTimeFilter(t)} className={`px-5 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${timeFilter === t ? 'bg-white text-indigo-600 shadow-sm border border-slate-100' : 'text-slate-400 hover:text-slate-600'}`}>{t}</button>
                  ))}
                </div>
             </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
             <div className="lg:col-span-3 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                   ...
                   {intelItems.length === 0 && <p className="text-sm font-bold text-slate-400 italic py-10">No intelligence hits for this category period.</p>}
                </div>
             </div>
                      ...
                      {[
                        ...
                        <div key={i} className="space-y-2">
                           <div className="flex justify-between items-center text-xs">
                              <span className="font-bold text-slate-400 uppercase tracking-tighter">{r.label}</span>
        ...
        {activeModTab === 'strategy' && (
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
             ...
             <div className="lg:col-span-1 bg-white rounded-3xl border border-slate-100 flex flex-col h-[600px] shadow-sm relative overflow-hidden">
                <div className="p-4 border-b border-slate-50 flex items-center gap-3 bg-indigo-50/30">
                   <div className="bg-indigo-100 p-2 rounded-xl text-indigo-600"><BrainCircuit size={18}/></div>
                   <div>
                      <h4 className="text-xs font-black text-slate-800 uppercase tracking-tight">Strategy Copilot</h4>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4 text-xs" ref={scrollRef}>
                   {strategyHistory.map((msg, i) => (
                     <div key={i} className={`flex ${msg.role === 'ai' ? 'justify-start' : 'justify-end'}`}>
                        <div className={`max-w-[90%] p-3 rounded-2xl ${msg.role === 'ai' ? 'bg-slate-50 text-slate-700 border border-slate-100 rounded-tl-none' : 'bg-indigo-600 text-white rounded-tr-none'}`}>
                   ...
                   {isCopilotTyping && (
                      <div className="flex justify-start">
                         <div className="p-3 bg-slate-50 border border-slate-100 rounded-2xl rounded-tl-none flex gap-1.5 animate-pulse">
                ...
                <div className="p-4 border-t border-slate-50 bg-slate-50/50">
                   <div className="relative flex items-center">
                      <textarea
                        ...
                        className="w-full bg-white border border-slate-200 rounded-xl pl-4 pr-12 py-3 text-xs font-medium focus:border-indigo-400 outline-none transition-all resize-none"
                   ...
                   <div className="flex gap-2 mt-3">
                      <button className="flex-1 bg-white border border-slate-200 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest text-slate-500 hover:bg-indigo-50 hover:text-indigo-600 transition-all" onClick={() => setCopilotInput('Summarize this strategy')}>Summarize</button>
                      <button className="flex-1 bg-white border border-slate-200 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest text-slate-500 hover:bg-indigo-50 hover:text-indigo-600 transition-all" onClick={() => setCopilotInput('Analyze Risks')}>Risk Check</button>
                   </div>
                </div>
             </div>

             {/* Right Pane - Workbook Viewer */}
             <div className="lg:col-span-3 bg-slate-50 rounded-3xl border border-slate-100 overflow-y-auto h-[600px] shadow-inner relative p-8">
                <div className="bg-white w-full max-w-[800px] mx-auto min-h-full shadow-lg border border-slate-100 p-12 space-y-10 relative">
                   ...
                   <div className="border-b-4 border-slate-900 pb-8">
                      <h1 className="text-4xl font-black text-slate-900 tracking-tighter mb-2 uppercase">Category Strategy Workbook</h1>
                      <div className="flex justify-between items-end">
                         <p className="text-sm font-bold text-indigo-600 uppercase tracking-widest">Confidential Strategy Document v2.4</p>
                         <div className="text-right">
                            <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1">Fiscal Target Category</p>
                            <p className="text-xl font-black text-slate-800">{selectedCategory?.name}</p>
                         </div>
                      </div>
                   </div>

                   <div className="grid grid-cols-3 gap-6 text-xs border-b border-slate-100 pb-8">
                      <div>
                         <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1">Strategy Lead</p>
                         <p className="font-bold text-slate-800 underline decoration-indigo-200 decoration-2">{categoryOwner}</p>
                      </div>
                      <div>
                         <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1">Last AI Review</p>
                         <p className="font-bold text-slate-800">Just Now</p>
                      </div>
                      <div className="flex justify-end gap-3">
                         <button className="flex items-center gap-1.5 bg-slate-900 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-600 transition-all shadow-lg shadow-slate-200">
                            <FileDown size={14}/> Export
                         </button>
                         <button className="flex items-center gap-1.5 bg-slate-100 text-slate-600 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-200 transition-all">
                   ...
                   <div className="space-y-12">
                      {strategyLoading ? (
                        <div className="flex flex-col items-center justify-center py-20 gap-4 animate-pulse">
                           ...
                           <p className="text-sm font-bold text-slate-400 uppercase tracking-widest">Synchronizing Intelligence Blocks...</p>
                        </div>
                        ...
                        <div key={idx} className="group relative pl-8 border-l-2 border-transparent hover:border-indigo-100 transition-colors">
                           <div className="absolute left-0 top-0 -translate-x-1/2 w-2 h-2 rounded-full bg-indigo-200 group-hover:scale-150 transition-transform"/>
                           <h3 className="text-lg font-black text-slate-800 mb-4 flex items-center gap-3">
                              ...
                              <span className="uppercase tracking-tight underline decoration-slate-100 decoration-4 underline-offset-4 group-hover:decoration-indigo-100 transition-all">{typeof sec === 'string' ? `Section ${idx+1}` : sec.title}</span>
                           </h3>
                           <div className="text-sm font-medium text-slate-600 leading-relaxed max-w-[640px]">
                      ...
                      {strategySections.length === 0 && !strategyLoading && (
                        <div className="text-center py-20 bg-slate-50 rounded-3xl border-2 border-dashed border-slate-100">
                           ...
                           <h4 className="text-lg font-black text-slate-400">Strategy Workspace Empty</h4>
                           <p className="text-sm font-bold text-slate-300 mt-2 uppercase tracking-widest">Instruct the Copilot to generate your first draft</p>
                        </div>
                      )}
                   </div>

                   <div className="pt-20 border-t border-slate-100 flex flex-col items-center opacity-30">
                      <p className="text-[10px] font-black uppercase tracking-[0.5em] text-slate-400">Workbook End · Page 1 of 1</p>
                   </div>
                </div>
             </div>
          </div>
        ...
        <Modal onClose={() => setVendorModal(false)} maxWidth="max-w-4xl">
          <div className="p-6 space-y-5 flex flex-col h-full">
            <div className="flex justify-between items-center">
              <div>
                <h3 className="text-xl font-black text-slate-800">Vendor Performance Scoring</h3>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Metric-level Performance Breakdown</p>
              </div>
              <button onClick={() => setVendorModal(false)} className="text-slate-400 hover:text-slate-600 p-2 hover:bg-slate-100 rounded-full transition-all"><X size={20}/></button>
            </div>
            <div className="bg-slate-50 p-2.5 rounded-xl border border-slate-200 flex items-center gap-3">
             ...
             <div className="flex-1 min-h-0 overflow-hidden rounded-2xl border border-slate-100 bg-white">
                     ...
                     <tbody className="divide-y divide-slate-50">
                        {filteredVendorScores.map((v, i) => (
                          <tr key={i} className="hover:bg-emerald-50/30 transition-colors">
                            <td className="px-4 py-3 font-bold text-slate-700">{v.vendor}</td>
                            ...
                            <td className="px-4 py-3 text-center font-black text-slate-900">{v.overall}</td>
                            {Object.keys(kpiData?.performance_weights || {}).map(k => (
                              <td key={k} className="px-4 py-3 text-center font-bold text-slate-500">
                                {v.breakdown?.[k] || '—'}
                              </td>
                            ))}
                          </tr>
      ...
      {riskModal && (
        <Modal onClose={() => setRiskModal(false)} maxWidth="max-w-5xl">
          <div className="p-6 space-y-5 flex flex-col h-full">
            <h3 className="text-xl font-black text-slate-800">Supplier Risk Matrix</h3>
            <div className="bg-slate-50 p-2.5 rounded-xl border border-slate-200 flex items-center gap-3">
              <Search size={14} className="text-slate-400 ml-2" />
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {filteredVendorRisk.map((v, i) => (
                      <tr key={i} className="hover:bg-emerald-50/30 transition-colors">
                        ...
                        <td className="px-4 py-3 text-center font-black text-slate-900">{v.overall_risk}</td>
                        {Object.keys(kpiData?.risk_weights || {}).map(k => (
                          <td key={k} className="px-4 py-3 text-center font-bold text-slate-500">
                            {v.breakdown?.[k] || '—'}
                          </td>
        ...
        <Modal onClose={() => setSelectedTask(null)} maxWidth="max-w-2xl">
          <div className="border-b border-slate-200 px-6 py-4 flex items-center gap-3 bg-slate-50/50">
            ...
            <div className="flex-1">
              <h3 className="text-lg font-black text-slate-800 tracking-tight">{selectedTask.desc}</h3>
              <div className="flex items-center gap-2 mt-0.5">
                <UrgencyBadge urgency={selectedTask.urgency} />
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{selectedTask.requester} · {selectedTask.pr_id || 'GENERAL'}</span>
              </div>
            </div>
            <button onClick={() => setSelectedTask(null)} className="text-slate-400 hover:text-slate-600 p-2"><X size={20}/></button>
          </div>
             ...
             <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5 space-y-4">
                <div className="flex justify-between items-center bg-white rounded-xl p-4 shadow-sm border border-slate-100">
                   <div>
                     ...
                     <p className="text-2xl font-black text-slate-900">₹{selectedTask.amount_cr || '0'} Cr</p>
                   </div>
                ...
                <div className="space-y-1.5">
                   <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 flex items-center gap-1"><MessageSquare size={10}/> Requester Justification</p>
                   <p className="text-sm font-medium text-slate-600 leading-relaxed italic">"Integrated supply chain buffer replenishment for Q2 operational continuity. Required for high-priority production line expansion."</p>
                </div>
             </div>
             <div className="space-y-3">
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Decision Comments (CPO)</label>
                <textarea rows={3} value={approvalNote} onChange={e => setApprovalNote(e.target.value)} placeholder="Enter formal decision notes..." className="w-full text-sm font-medium p-4 border border-slate-200 rounded-2xl outline-none focus:border-indigo-400 focus:ring-4 focus:ring-indigo-100 transition-all bg-slate-50" />
      ...
      {pipModal && (
        <Modal onClose={() => setPipModal(false)} maxWidth="max-w-4xl">
           <div className="p-8 space-y-8">
              <div className="flex justify-between items-center">
                 <div>
                    <h3 className="text-2xl font-black text-slate-800 flex items-center gap-2"><Activity size={24} className="text-indigo-600" /> P•I•P Intelligence: Peer Benchmarking</h3>
                    <p className="text-sm font-bold text-slate-400 uppercase tracking-widest">Internal Category Performance Index</p>
                 </div>
                 <button onClick={() => setPipModal(false)} className="p-2 hover:bg-slate-100 rounded-full transition-all text-slate-400"><X size={24} /></button>
              </div>
                 ...
                 <div className="md:col-span-2 grid grid-cols-2 gap-4">
                    {[
                      ...
                      <div key={i} className="bg-slate-50 border border-slate-100 rounded-2xl p-5">
                         ...
                         <p className="text-2xl font-black text-slate-800">{p.val}</p>
              ...
              <div className="space-y-4">
                 ...
                 <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-3">
                       <h5 className="text-xs font-black text-slate-800 uppercase tracking-widest">How we calculate index</h5>
                       <p className="text-sm text-slate-500 leading-relaxed font-medium">The PIP index is a weighted benchmark consisting of 4 pillars: Operational Efficiency (30%), Strategic Savings (30%), Compliance Integrity (20%), and Market Agility (20%).</p>
                    </div>
           ...
           <div className="flex flex-col h-full bg-white">
              <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                 <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="text-2xl font-black text-slate-800">Intelligence Opportunity Drill-Down</h3>
                      <p className="text-sm font-bold text-slate-400 uppercase tracking-widest">{selectedCategory?.name} · Global Market Dynamics</p>
                    </div>
                 </div>
                 <button onClick={() => setSelectedIntel(null)} className="p-3 hover:bg-white hover:shadow-md rounded-full transition-all text-slate-400"><X size={24}/></button>
              </div>
              <div className="flex-1 overflow-y-auto p-8 space-y-8">
                 <div className={`p-6 rounded-3xl border-l-[8px] shadow-sm flex flex-col gap-3 ${selectedIntel.impact === 'High' ? 'bg-rose-50 border-rose-500' : 'bg-amber-50 border-amber-500'}`}>
                    <div className="flex justify-between items-center">
                       <h4 className="text-xl font-black text-slate-900">{selectedIntel.title}</h4>
                       <ImpactBadge impact={selectedIntel.impact} />
                    </div>
                    <p className="text-sm text-slate-700 font-medium leading-relaxed italic">"Dynamic market shifts detected in the Southeast Asian corridor. This signal suggests a 12% price hike likelihood within 6 months."</p>
                 </div>
                 
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm space-y-6">
                       <h5 className="text-[11px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2"><Target size={14}/> Impact Assessment & Quantification</h5>
                       <div className="grid grid-cols-2 gap-4">
                          <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100">
                             <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Financial Loss Risk</p>
                             <p className="text-2xl font-black text-rose-600">₹{selectedIntel.quantified_value || '1.4'} Cr</p>
                          </div>
                          <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100">
                             <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Business Impact</p>
                             <p className="text-2xl font-black text-indigo-600">{selectedIntel.business_impact || 'Moderate'}</p>
                          </div>
                       </div>
                       <div className="space-y-4">
                          <p className="text-[11px] font-black text-slate-500 uppercase tracking-widest">Recommended Mitigation Plan</p>
    ...
    <div className="space-y-6">
       <div className="flex bg-slate-50 p-1.5 rounded-xl border border-slate-100">
         {['price','sourcing'].map(t => (
           <button key={t} onClick={() => setActiveTab(t)} className={`flex-1 py-2 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all ${activeTab === t ? 'bg-white shadow-md text-indigo-600' : 'text-slate-400'}`}>{t === 'price' ? 'Price Movement' : 'Alt sourcing'}</button>
         ))}
       </div>
       
       {activeTab === 'price' && (
         <div className="space-y-4 animate-in fade-in duration-300">
            <h6 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Historical & Forecast (INR / MT)</h6>
       ...
       {activeTab === 'sourcing' && (
         <div className="space-y-4 animate-in fade-in duration-300">
            <h6 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Recommended Secondary Sources</h6>
            <div className="space-y-3">
               {ALT_SOURCES.map((s, i) => (
                 <div key={i} className="bg-slate-50 border border-slate-100 rounded-2xl p-4 flex justify-between items-center">
                    <div>
                       <p className="text-xs font-black text-slate-800">{s.vendor}</p>
                       <p className="text-[10px] font-bold text-slate-400">Reliability Index: {s.reliability}%</p>
                    </div>
                    <div className="text-right">
