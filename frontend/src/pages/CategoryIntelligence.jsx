    ...
    <div className="p-6 space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex flex-col gap-1">
          ...
          <h1 className="text-3xl font-black tracking-tight text-slate-900">Category Strategy</h1>
        ...
        <div className="flex items-center gap-2">
          <div className="relative w-64 transition-all focus-within:w-72">
            ...
            <input 
              ...
              className="h-10 w-full pl-9 pr-4 rounded-md border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-100 hover:border-slate-300 transition-all bg-white shadow-sm"
              value={localSearch}
              onChange={e => setLocalSearch(e.target.value)}
            />
          </div>
          <Button 
            className="h-10 bg-indigo-600 hover:bg-indigo-700 shadow-lg shadow-indigo-600/20"
              ...
              <Card key={cat.id} className="border-none shadow-xl shadow-slate-200/40 bg-white group hover:-translate-y-1 transition-all duration-300">
                <CardHeader className="pb-4">
                  <div className="flex items-center justify-between mb-2">
                    <Badge variant="secondary" className="bg-indigo-50 text-indigo-600 font-bold border-none uppercase text-[9px] tracking-tight">
                      {cat.id % 2 === 0 ? 'Direct Material' : 'Indirect Services'}
                    </Badge>
                    <TrendingUp className="h-4 w-4 text-emerald-500" />
                  </div>
                  <CardTitle className="text-xl font-black text-slate-900">{cat.name}</CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  ...
                  <Button variant="ghost" className="w-full justify-between group-hover:bg-indigo-50 transition-colors mt-2 font-bold text-xs text-indigo-600">
                    View Strategy Docs
