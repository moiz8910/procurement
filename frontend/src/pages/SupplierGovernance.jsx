    ...
    <div className="p-6 space-y-6 animate-in slide-in-from-right-4 duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-sm text-neutral-500 font-medium">Monitor compliance, ESG scores, and performance health.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="border-none shadow-none bg-neutral-50/50">
          <CardContent className="pt-6">
            <div className="text-xs font-bold text-neutral-400 uppercase mb-1">Total Vendors</div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-neutral-50 border-b border-neutral-100">
                    <th className="text-left p-4 font-bold text-neutral-500 uppercase text-[10px] tracking-widest pl-6">Supplier</th>
                    <th className="text-left p-4 font-bold text-neutral-500 uppercase text-[10px] tracking-widest">Category</th>
                    <th className="text-left p-4 font-bold text-neutral-500 uppercase text-[10px] tracking-widest">Risk Score</th>
                    <th className="text-left p-4 font-bold text-neutral-500 uppercase text-[10px] tracking-widest">Status</th>
                    <th className="text-right p-4 font-bold text-neutral-500 uppercase text-[10px] tracking-widest pr-6">Detail</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-neutral-100">
                  {filteredVendors.slice(0, 10).map((vendor) => (
                    <tr key={vendor.id} className="group hover:bg-neutral-50 transition-colors">
                      ...
                      <td className="p-4 text-neutral-600 font-medium">Bauxite Mining</td>
                      <td className="p-4">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[10px] font-bold text-neutral-400">{85 - (vendor.id * 2)}%</span>
                        </div>
                      </td>
                      <td className="p-4">{getRiskBadge(85 - (vendor.id * 2))}</td>
                      <td className="p-4 text-right pr-6">
                        <Button variant="ghost" size="icon" className="h-8 w-8 text-neutral-300 group-hover:text-teal-600 transition-colors">
