import React, { useState } from 'react';
import CategoryModule from './CategoryModule';
import CategoryIntelligence from './CategoryIntelligence';
import StrategyDefinitionModule from './StrategyDefinitionModule';
import { Layers, Globe, FileSignature, Target } from 'lucide-react';

const SpendOpportunities = () => (
  <div className="p-8 flex flex-col items-center justify-center h-full text-center bg-neutral-50 animate-in fade-in duration-500">
    <Target size={64} className="text-emerald-200 mb-6" />
    <h3 className="text-3xl font-black text-blue-900 tracking-tight">Market Yield Opportunities</h3>
    <p className="text-neutral-500 font-medium max-w-lg mt-4 text-sm leading-relaxed">
      AI-driven spend consolidation and arbitrage opportunities. System is currently scanning global commodities indices.
    </p>
  </div>
);

const CategoryMasterModule = () => {
  const [activeTab, setActiveTab] = useState('analytics');

  const tabs = [
    { id: 'analytics', label: 'Category Analytics', icon: Layers },
    { id: 'intelligence', label: 'Market Intelligence', icon: Globe },
    { id: 'strategy', label: 'Strategy Builder', icon: FileSignature },
    { id: 'opportunities', label: 'Spend Opportunities', icon: Target }
  ];

  const renderContent = () => {
    switch (activeTab) {
      case 'analytics': return <CategoryModule />;
      case 'intelligence': return <CategoryIntelligence />;
      case 'strategy': return <StrategyDefinitionModule />;
      case 'opportunities': return <SpendOpportunities />;
      default: return <CategoryModule />;
    }
  };

  return (
    <div className="flex flex-col h-full w-full">
      <div className="bg-white border-b border-neutral-100 shrink-0 sticky top-0 z-10 w-full px-6 pt-4 shadow-sm">
         <div className="flex items-center gap-8 overflow-x-auto no-scrollbar">
           {tabs.map(t => (
             <button
               key={t.id}
               onClick={() => setActiveTab(t.id)}
               className={`flex items-center gap-2 pb-3 border-b-[3px] font-black text-[11px] transition-all uppercase tracking-[0.1em] whitespace-nowrap ${
                 activeTab === t.id 
                 ? 'border-emerald-600 text-emerald-700' 
                 : 'border-transparent text-neutral-400 hover:text-neutral-700 hover:border-neutral-200'
               }`}
             >
               <t.icon size={16} className={activeTab === t.id ? "text-emerald-600" : "text-neutral-400"} /> 
               {t.label}
             </button>
           ))}
         </div>
      </div>
      <div className="flex-1 overflow-x-hidden min-h-0 bg-neutral-50/30">
         {renderContent()}
      </div>
    </div>
  );
};

export default CategoryMasterModule;
